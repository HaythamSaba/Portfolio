...
let progressValue = document.querySelectorAll(".progress .progress-value");
  ...
  window.onscroll = function () {
    if (window.scrollY >= skills.offsetTop - 50) {
      if (!skillsSectionReached) {
        ...
        circularProgress.forEach((el, i) => {
          let target = parseInt(el.dataset.progress);
          let start = 0;

          let progress = setInterval(() => {
            start++;
            if (start >= target) clearInterval(progress);

            // Update circular background
            el.style.background = `conic-gradient(var(--two-main-color) ${
              start * 3.6
            }deg, var(--one-main-color) 0deg)`;

            // Update progress text if it exists
            if (progressValue[i]) {
              progressValue[i].textContent = `${start}%`;
            }
          }, 20); // ~2 seconds for 100 steps
        });
